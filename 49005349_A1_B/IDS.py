        ...
        for packet in packets:
            for rule in rules:
                if rule['protocol'] == "tcp" and packet.haslayer(scapy.TCP):
                    ...
                    if src_ip == rule['src_ip'] and (rule['dst_ip'] == "any" or dst_ip == rule['dst_ip']):
                        tcp_flags = packet[scapy.TCP].flags
                        ...
                        if rule.get('flags'):
                            if 'S' in rule['flags'] and tcp_flags == 'S':
                                if rule.get('detection_filter'):
                                    ...
                                    if src_ip not in detection_filter_state:
                                        detection_filter_state[src_ip] = {'packets': [], 'last_count': 0}
                                    
                                    detection_filter_state[src_ip]['packets'].append(packet.time)
                                    detection_filter_state[src_ip]['packets'] = [t for t in detection_filter_state[src_ip]['packets'] if packet.time - t <= seconds]
                                    
                                    current_count = len(detection_filter_state[src_ip]['packets'])
                                    alerts = max(0, current_count - max(count, detection_filter_state[src_ip]['last_count']))
                                    detection_filter_state[src_ip]['last_count'] = current_count
                                    
                                    for _ in range(alerts):
                                        log_alert(log_file, rule['msg'])
                            elif ('A' in rule['flags'] and tcp_flags == 'A') or \
                                 ('R' in rule['flags'] and tcp_flags == 'R'):
                                log_alert(log_file, rule['msg'])
                            elif 'F' in rule['flags'] and tcp_flags == 'F' and rule.get('content'):
                                if packet.haslayer(scapy.Raw) and rule['content'] in packet[scapy.Raw].load.decode(errors='ignore'):
                                    if rule.get('detection_filter'):
                                        count = int(rule['detection_filter'][0].strip().split()[1])
                                        seconds = int(rule['detection_filter'][1].strip().split()[1])
                                        if check_detection_filter(packet.time, src_ip, count, seconds, detection_filter_state):
                                            log_alert(log_file, rule['msg'])
                        elif rule.get('content'):
                            if packet.haslayer(scapy.Raw) and rule['content'] in packet[scapy.Raw].load.decode(errors='ignore'):
                                log_alert(log_file, rule['msg'])
                    ...
                    if (rule['src_ip'] == "192.168.0.33" and packet[scapy.IP].src == "192.168.0.33") or rule['src_ip'] == "any":
                        log_alert(log_file, rule['msg'])
                
                elif rule['protocol'] == "ip" and packet.haslayer(scapy.IP):
                    if (packet[scapy.IP].src == rule['src_ip'] and packet[scapy.IP].dst == rule['dst_ip']) or \
                       (packet[scapy.IP].dst == rule['dst_ip'] and packet.haslayer(scapy.TCP) and packet[scapy.TCP].dport == 6666):
                        log_alert(log_file, rule['msg'])
                    ...
                    if packet[scapy.IP].src == rule['src_ip'] and packet[scapy.IP].dst == rule['dst_ip']:
                        if rule.get('content'):
                            if packet.haslayer(scapy.Raw) and rule['content'] in packet[scapy.Raw].load.decode(errors='ignore'):
                                log_alert(log_file, rule['msg'])
                        elif (packet[scapy.UDP].sport == int(rule['src_port']) and
                              packet[scapy.UDP].dport == int(rule['dst_port'])):
                            log_alert(log_file, rule['msg'])
